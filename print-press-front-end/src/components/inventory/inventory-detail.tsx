...
import { ArrowLeft, Package, AlertTriangle, DollarSign, ShoppingCart, Edit } from 'lucide-react';
...
export const InventoryDetail: React.FC<InventoryDetailProps> = ({ itemId }) => {
  ...
  const renderAttributes = (item: InventoryItem) => {
    ...
    if (item.category === 'Paper') {
      ...
      if (attributes.sheets_per_unit) commonAttributes.push(`${attributes.sheets_per_unit} sheets/unit`);
    ...
    return commonAttributes.length > 0 ? commonAttributes.join(' • ') : 'No specific attributes';
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{item.display_stock}</p>
                <p className="text-xs text-gray-500">{item.current_stock} {item.unit_of_measure}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{item.threshold} {item.unit_of_measure}</p>
                <div className={`mt-2 px-2 py-1 rounded-full text-xs font-medium inline-flex items-center gap-1 ${getStockStatusColor(item.stock_status)}`}>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-xs text-gray-500">per {item.unit_of_measure}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {formatCurrency((item.stock_value || item.current_stock * item.unit_cost) || 0)}
                </p>
          ...
          <CardContent className="space-y-4">
            ...
            {item.selling_price && (
              <div>
                ...
                <p className="text-gray-900">{formatCurrency(item.selling_price)}</p>
              </div>
            )}
            {item.reorder_quantity && (
              <div>
                <p className="text-sm font-medium text-gray-500">Reorder Quantity</p>
                <p className="text-gray-900">{item.reorder_quantity} {item.unit_of_measure}</p>
              </div>
            )}
            <div>
          ...
          <CardContent className="space-y-4">
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-lg font-bold text-gray-900">{item.display_stock}</span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">{formatCurrency(item.unit_cost)}</p>
              </div>
              <div>
            ...
            {item.stock_percentage !== undefined && (
              <div>
                <p className="text-sm font-medium text-gray-500">Stock Percentage</p>
                <p className="text-lg font-semibold text-gray-900">{item.stock_percentage.toFixed(1)}%</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <PurchaseItemModal
        ...
        onSuccess={() => {
          // Refresh item data
          inventoryApi.getInventoryItem(itemId).then(({ item: updatedItem }) => {
            setItem(updatedItem);
          });
        }}
      />
