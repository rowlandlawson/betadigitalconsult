...
export const PurchaseItemModal: React.FC<PurchaseItemModalProps> = ({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => {
      ...
      if (name === 'quantity' || name === 'unit_price') {
        const qty =
          name === 'quantity' ? parseFloat(value) : parseFloat(prev.quantity);
        const unitPrice =
          name === 'unit_price'
            ? parseFloat(value)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `Successfully purchased ${quantity} ${item.unit_of_measure} of ${item.material_name}`
      );
      onSuccess?.();
      onClose();
    } catch (err: any) {
          ...
          <CardContent className="space-y-4">
            ...
            <div className="p-3 bg-gray-50 rounded-md">
              ...
              <p className="text-lg font-semibold text-gray-900">
                {item.display_stock ||
                  `${Number(item.current_stock || 0).toFixed(2)} ${item.unit_of_measure}`}
              </p>
            </div>
                ...
                <p className="text-lg font-semibold text-blue-900">
                  {(() => {
                    try {
                      ...
                      if (
                        item.category === 'Paper' &&
                        item.attributes?.sheets_per_unit
                      ) {
                        const sheetsPerUnit =
                          parseInt(item.attributes.sheets_per_unit) || 500;
