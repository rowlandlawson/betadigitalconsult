...
export const JobDetail: React.FC<JobDetailProps> = ({ jobId, userRole }) => {
  ...
  const handleOpenReceiptModal = () => {
    if (!job || job.payments.length === 0) {
      toast.error(
        'No payments found for this job. Record a payment to generate a receipt.'
      );
      return;
    }
  ...
  const calculateFinancials = () => {
    ...
    const materialsCost =
      job.materials?.reduce((sum, m) => sum + Number(m.total_cost), 0) || 0;
    // Sum of Waste Cost
    const wasteCost =
      job.waste?.reduce((sum, w) => sum + Number(w.total_cost), 0) || 0;
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div className="flex justify-center sm:justify-start">
              <span
                className={`inline-flex items-center px-3 py-2 rounded-full text-sm font-medium ${getStatusColor(job.status)} w-fit`}
              >
                  ...
                  <div className="space-y-2">
                    {job.materials.map((material, index) => (
                      <div
                        ...
                        className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 border border-gray-200 rounded-lg gap-2"
                      >
                        <div className="text-center sm:text-left">
                ...
                {!job.waste || job.waste.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {job.waste.map((waste, index) => (
                      <div
                        key={waste.id || index}
                        className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 border border-red-100 bg-red-50 rounded-lg gap-2"
                      >
                        <div className="text-center sm:text-left">
                          <p className="font-medium text-gray-900 break-words">
                            {waste.description}
                          ...
                          <p className="text-sm text-gray-500 capitalize">
                            ...
                            {waste.quantity &&
                              waste.unit_cost &&
                              ` • ${waste.quantity} × ${formatCurrency(waste.unit_cost)}`}
                          </p>
                        </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`font-semibold text-sm sm:text-base ${job.balance > 0 ? 'text-red-600' : 'text-green-600'}`}
                  >
                ...
                {userRole === 'admin' && (
                  <div className="mt-4 pt-4 border-t-2 border-dashed border-gray-200">
                    <h4 className="font-medium text-gray-900 mb-2 text-center sm:text-left">
                      Job Profitability
                    </h4>
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between items-center pt-2 border-t border-gray-100 mt-2">
                        ...
                        <div className="text-right">
                          <span
                            className={`block font-bold text-base sm:text-lg ${financials.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {formatCurrency(financials.profit)}
                          </span>
                          <span
                            className={`text-xs ${financials.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
          ...
          <div className="relative z-10 w-full max-w-5xl max-h-[95vh] bg-white rounded-xl shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Select a payment to view its printable receipt.
                </p>
              </div>
              <Button
                variant="ghost"
