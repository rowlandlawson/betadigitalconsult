...
import { User, Banknote, Calendar, Plus } from 'lucide-react';
...
export const SalaryManager = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            // Filter only workers
            setWorkers(usersRes.data.users.filter((u: Worker) => u.role === 'worker'));
        } catch (error) {
                ...
                <Card className="lg:col-span-2">
                    ...
                    <CardContent>
                        {loading ? (
                            ...
                            <div className="text-center py-8 text-gray-500">No salary payments recorded yet.</div>
                        ) : (
                            <div className="space-y-4">
                                {payments.map((payment) => (
                                    <div key={payment.id} className="flex justify-between items-center p-4 border rounded-lg bg-gray-50">
                                        <div>
                                            <h4 className="font-semibold">{payment.worker_name}</h4>
                                            <p className="text-sm text-gray-500">
                                                {payment.payment_month} • Paid on {formatDate(payment.payment_date)}
                                            </p>
                                            {payment.notes && <p className="text-xs text-gray-400 mt-1">{payment.notes}</p>}
                                        </div>
                                        <div className="text-right">
                                            <p className="font-bold text-green-600">{formatCurrency(payment.amount)}</p>
                                            <p className="text-xs text-gray-400">By {payment.recorded_by_name}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
