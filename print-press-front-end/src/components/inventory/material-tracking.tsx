...
export const MaterialUsageTracker: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedMaterial || !sheetsUsed || Number(sheetsUsed) <= 0) {
      setMessage({ type: 'error', text: 'Please select a material and enter a valid number of sheets.' });
      return;
    }
    ...
    try {
      ...
      fetchInventory(); // Refresh list
        ...
        const errorMsg = err.response?.data?.error || 'Failed to record material usage.';
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        ...
        <p className="text-gray-600">Record material usage for production jobs.</p>
      </CardHeader>
      ...
      <form onSubmit={handleSubmit}>
            ...
            <div className="p-4 bg-gray-50 rounded-lg border">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium text-red-600">{formatCurrency(costOfUsage)}</p>
                </div>
                <div>
        ...
        <CardFooter className="flex justify-between">
            ...
            <Button type="submit" disabled={submitting || !selectedMaterial || !sheetsUsed}>
