...
class ReceiptService {
  ...
  addReceiptContent(doc, receipt) {
    const businessName = receipt.company_name || receipt.business_name || 'PRINTPRESS SUITE';
    const businessPhone = receipt.company_phone || receipt.business_phone || '+234 123 456 7890';
    const businessEmail = receipt.company_email || 'support@printpresssuite.com';
    const businessAddress = receipt.company_address || receipt.business_address || '123 Printing Street, Your City';
    ...
    doc.text(`Phone: ${businessPhone} • Email: ${businessEmail}`, { align: 'center' });
